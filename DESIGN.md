...
> **詳細は [docs/learning-cycle.md](docs/learning-cycle.md) を参照。**

**週1回の本番形式テストを軸に回す。** 週次テストは「実力を測る場」、日次ドリルは「実力を上げる場」として役割を分ける。

```
週次（土 or 日）
  志望校の本番と同じ大問構成・配点・制限時間で通しテスト
   ↓ 全問マーク式のため即時採点・即時分析
   ↓ 翌週の重点単元 3〜5個を決定
月〜金 デイリーループ
  朝 7:00 通知 →「今日のミッション（推定15分）」
   ↓ 苦手中心、ただし志望校の配点比率に沿ってバランス配分
   ↓ マーク・数値マークで即時採点（AI不使用）
  夜 21:00 通知 →「今日の振り返り」
   ↓
  21:30 保護者へ日次レポート自動送信
   ↓
再び週次テストへ（今週やったことが、来週測られる）
```

**この「今週やったこと」と「来週測られること」が一本の線でつながる構造が、学習ループの核心**であり、汎用の問題集アプリとの差になります。
...
| 画面 | パス | 内容 |
|------|------|------|
| ホーム | `/` | 受験まで残り◯日 / 今日のミッション / 今週の目標の進捗 / 次の週次テストまで |
| 今週の目標 | `/weekly` | 重点単元3〜5個と達成度バー、次のテストまでの残り日数 |
| 週次テスト | `/weekly/test` | 本番形式の受験画面（模試受験画面を流用） |
| 週次レポート | `/weekly/report` | スコア・志望校配点換算・合格率・伸びた単元・翌週の目標 |
...
- ホームは**「今なにをやればいいか」1つに絞る**。選択肢を並べると何もしないまま閉じられます。週次の情報（今週の目標・次のテスト）は**進捗の文脈**として下に置き、行動の選択肢を増やしません。
...
> 配信するのはマーク式2形式のみ（数学＝数値マーク / その他＝5択）で、記述・証明は扱いません。
> 対応できる入試方式は共通テスト型・私大マーク型であり、国公立二次などの記述型は再現できません。
> この制約はLP・オンボーディングで明示します（[learning-cycle.md](docs/learning-cycle.md) §1.2）。
>
> 誤答の選択肢・誤答値そのものに診断タグを埋め込むことで、§6.1 の誤答タイプ分類を
> AI呼び出しなしで決定論的に行います。採点にAIは使いません。
...
- **生成**: Claude に tool use で JSON Schema を強制し、`body_md` / `answer` / `choices`（誤答タグ付き）/ `explanation_md` を構造化出力。数式は KaTeX。
- **検証（必須）**: 生成とは**別セッション**で、問題文だけを渡して3回独立に解かせる。
  - 3回とも生成時の `answer` と一致 → `quality_score = 1.0` → 配信可
  - 2/3一致 → 人手レビューキューへ
  - それ以下 → 破棄して再生成（最大3回）
- **誤答の否定検証（マーク式で最重要）**: 4つの誤答選択肢を1つずつ「これは確実に誤りか」と判定させる。1つでも「正しい可能性がある」なら破棄。**AI生成マーク問題の最大の事故は「正解が2つある」こと**です（→ [question-format.md](docs/question-format.md) §5）。
...
**全問マーク式のため、採点にAIは一切使いません。** 採点待ちもコストも発生せず、結果がブレることもありません。

- **5択**: 選ばれた選択肢を照合。誤答なら、選択肢に事前に埋め込んだ誤答タグをそのまま引く。
- **数値マーク**: 表記ゆれ（全角・空白・未約分の分数）を正規化して照合。スロット単位で部分点を出す。誤答値マッピングに一致すれば誤答タイプが確定する。
...
> **配分の詳細は [docs/learning-cycle.md](docs/learning-cycle.md) §4 を参照。**

毎朝、以下の3層から**推定10〜30分**（学年と残り日数で変動）に収まるようタスクを選びます。

| 層 | 目的 | 既定の配分 |
|---|---|---|
| 復習期限（`review_queue` の `due_at <= 今日`） | 忘却の防止 | 40% |
| 今週の重点単元（週次テストの分析で決定） | 弱点の克服 | 40% |
| 維持（得意科目の劣化防止） | 落ちたことに気づくのが手遅れになるのを防ぐ | 20% |

**科目配分の基準は「均等」ではなく「志望校の配点比率」**です。英語200点・数学100点の大学なら英語に多く割くのが正しく、均等配分は得点を最大化しません。この比率を土台に、弱点度で傾けます。

偏りを防ぐハードガード（最低2科目にまたがる、4日以上連続で0問にしない）は learning-cycle.md §4.3 で定義します。
...
| 用途 | モデル | 理由 |
|------|--------|------|
| 問題生成（誤答選択肢の設計を含む）・品質ゲートの検証 | `claude-opus-5` | 品質が直接プロダクト価値。ここはケチらない |
| 解説生成・ミニ講義・保護者レポート文面 | `claude-sonnet-5` | 品質とコストのバランス |
| 単元タグ付け・通知文 | `claude-haiku-4-5-20251001` | 高頻度・軽量。件数が出る |

**採点にAIは使いません**（§5.4）。誤答タイプの分類も、誤答選択肢・誤答値に事前に埋め込んだタグを引くだけなのでAI呼び出しは発生しません。AIを使うのは**問題を作るときだけ**で、生成した問題はプールに永続化されるためコストは初回のみです。

- **Prompt caching** を活用: 単元タグ体系・出題ブループリント・誤答タグ体系は共通接頭辞に置いてキャッシュする。問題生成のトークン単価が大きく下がります。
...
| **1** | 3週 | 単元タグ体系、マーク/数値マークの生成＋品質ゲート、日次ドリルの出題・自動採点（要件3） | 自動採点の問題が生成され、毎日出題できる |
| **2** | 2週 | 習熟度推定、誤答タイプ分類、苦手可視化、復習ループ、FSRS（要件4） | 弱点が出て、復習セッションが回る |
| **3** | 2週 | 週次テスト（本番と同構成の通しテスト）、週次分析→翌週の課題化、科目ローテーション | 週次サイクルが1周する |
| **4** | 2週 | Push通知（朝夜＋週次）、保護者メール、Cron基盤（要件7・8） | 朝夜の通知と日次レポートが実運用で届く |
| **5** | 1週 | 友達紹介、不正対策（要件6） | 招待→成立→特典付与が通る |
| **6** | — | 成績推移・志望校判定、課金、指導者向け機能 | |

**Phase 0 で PWA を先に入れる理由**: iOS の Push がホーム画面追加を前提とするため（§9.1）、後回しにすると通知が届かず設計をやり直すことになります。

**Phase 1〜2 で先に日次ドリルを回す理由**: 週次テスト（Phase 3）は日次ドリルと同じ出題形式・同じ採点ロジックを使うため、先に日次側を作れば週次側は「本番と同じ構成で通しに組む」だけで済みます。順序を逆にすると、検証されていない出題エンジンで本番形式を組むことになります。

**先に手を付けるべきリスク**: ①問題生成の品質（§5.3 の検証ゲートを Phase 1 の最初に作る。ここが通らないとプロダクトが成立しない）、②学校マスタと過去問メタデータの収集コスト（第一志望になりやすい上位100校に絞って着手）。
...
2. **科目範囲**: 全5教科か、数学・英語から始めるか（マーク式の誤答選択肢を設計する難度は 国語 > 英語 > 数学 の順に高い）
